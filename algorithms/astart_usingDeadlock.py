...
def preProcessing(array_2d, distance_grid):
    ...
    for i in range(len(array_2d)):
        for j in range(len(array_2d[0])):
            ...
            if array_2d[i][j] == "$" or array_2d[i][j] == "*":  # Hộp
...
def is4Box(cur_box, update_pos, movement, goal_positions):
    x, y = cur_box
    dx, dy = movement
    tmpx, tmpy = dx + x, dy + y
...
#print("PREPROSSING FINISH")
#for d in array_2d:
#    print(d)
path, n = a_star((8, 11), goal_positions, box_positions, distance_grid, rock_weight, n)
